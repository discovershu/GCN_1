...
adj, features0, y_train1, _, _, _, _, test_mask0 = read_data.load_data_dc(week=0, test_rat=FLAGS.test_rat, hour=0)
# Some preprocessing
    ...
    for j in range(6):
        ...
        for k in range(window_slide):
            ...
            for epoch in range(FLAGS.epochs):
                ...
                # load data
                _, features, y_train, _, _, train_mask, _, _, _ = read_data.load_data_dc(week=p, test_rat=FLAGS.test_rat,
                                                                                        hour=j)
            ...
            for i in range(weeks):
                i = i + k
                _, features, _, _, y_test, _, _, test_mask = read_data.load_data_dc(week=i, test_rat=FLAGS.test_rat,
                                                                                      hour=j)
                # features = preprocess_features(features)
                features = sparse_to_tuple(features)
