...
adj, features0, y_train1, _, _, _, _, test_mask0 = read_data.load_data_direct(week=0, test_num=FLAGS.test_num, hour=0)
# Some preprocessing
    ...
    for j in range(6):
        ...
        for k in range(window_slide):
            ...
            for epoch in range(FLAGS.epochs):
                ...
                # load data
                _, features, y_train, _, _, train_mask, _, _ = read_data.load_data_direct(week=p, test_num=FLAGS.test_num,
                                                                                        hour=j)
            ...
            for i in range(weeks):
                i = i + k
                _, features, _, _, y_test, _, _, test_mask = read_data.load_data_direct(week=i, test_num=FLAGS.test_num,
                                                                                      hour=j)
                # features = preprocess_features(features)
                features = sparse_to_tuple(features)
            ...
            acc.append(acc_test)
            np.save("./result/opinion_0.3_dc_T11_gcn_6hour_Fr_direct.npy", opinion_e)
            t2 = time.time()
            print(t2 - t1)
