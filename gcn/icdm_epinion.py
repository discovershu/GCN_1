...
adj, features0, y_train1, _, _, _, _, test_mask0, _, _, _ = read_data.load_data_epinion(week=0, T=10)
# Some preprocessing
    ...
    for j in range(1):
        ...
        for k in range(1):
            ...
            for epoch in range(FLAGS.epochs):
                ...
                # load data
                _, features, y_train, _, _, train_mask, _, _, _, _, _ = read_data.load_data_epinion(week=p, T=weeks)
            ...
            for i in range(weeks):
                i = i + k
                _, features, _, _, y_test, _, _, test_mask, test_index, belief, uncertain = read_data.load_data_epinion(week=i, T=weeks)
                # features = preprocess_features(features)
                features = sparse_to_tuple(features)
