...
flags.DEFINE_integer('max_degree', 0, 'Maximum Chebyshev polynomial degree.')
        ...
        for k in range(1):
            ...
            for epoch in range(FLAGS.epochs):
                ...
                if np.mod(epoch, 1000) == 0:
                    ...
                    for i in range(100):
                        # i = i + k
                        _, features, y_test, test_mask = read_data.load_test_data_synthetic(i, label_test,
                                                                                            features_test)
                        # features = preprocess_features(features)
                        features = sparse_to_tuple(features)
                    ...
                    print("epoch=", epoch, "Test precision=", "{:.5f}".format(np.mean(precision)), "Test Recall=",
                          "{:.5f}".format(np.mean(recall)),
