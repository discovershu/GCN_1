...
flags.DEFINE_integer('test_num', 152, 'Number of test nodes.')

# Load data
# adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data(FLAGS.dataset)
adj, features0, y_train1, _, _, _, _, test_mask0 = read_data.load_data_eopinion(week=0)
# Some preprocessing
...
weeks = 2  # T = weeks
window_slide = 20 - weeks  # window size
opinion_e = []
m = 18
    ...
    for j in range(1):
        ...
        for k in range(1):
            ...
            for epoch in range(FLAGS.epochs):
                ...
                p = np.mod(epoch, weeks) + m
                _, features, y_train, _, _, train_mask, _, _ = read_data.load_data_eopinion(week=p)
                ...
                if len(cost_train) == 50:
                    cost_epoch_85.append(np.mean(cost_train))
                    train_acc_85.append(np.mean(train_acc))
                    cost_train = []
                    train_acc = []
                    epoch_85 += 1
                    print("Epoch:", '%04d' % (epoch_85), "train_loss=", "{:.5f}".format(cost_epoch_85[-1]),
                          "train_acc=", "{:.5f}".format(train_acc_85[-1] * 100), "time=",
                          "{:.5f}".format((time.time() - t) * weeks))
            ...
            for i in range(weeks):
                i = i + m
                _, features, _, _, y_test, _, _, test_mask = read_data.load_data_eopinion(week=i)
            ...
            np.save("/network/rit/lab/ceashpc/xujiang/eopinion/result_gcn/eopinion_test_0.2_graph10000_T2_18.npy",
                    opinion_e)
            t2 = time.time()
            print(t2 - t1)
            print("weeks:", '%02d' % weeks, "window:", '%02d' % k, "Mean opinion error=",
