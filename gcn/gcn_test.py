...
adj, features0, y_train1, _, _, _, _, test_mask0 = read_data.load_tmp_data(0.9)
# Some preprocessing
...
with tf.Session(config=config) as sess:
    _, features, y_train, _, y_test, train_mask, _, test_mask = read_data.load_tmp_data(0.9)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        print("Epoch:", '%04d' % (epoch), "train_loss=", "{:.5f}".format(outs[1]),
              "train_acc=", "{:.5f}".format(outs[2] * 100))

    print("Optimization Finished!")
    feed_dict = construct_feed_dict(features, support, y_test, test_mask, placeholders)
    outs = sess.run([model.loss, model.loss, model.accuracy], feed_dict=feed_dict)
    print("Test=", "{:.5f}".format(outs[1]), "test_acc=", "{:.5f}".format(outs[2] * 100))
