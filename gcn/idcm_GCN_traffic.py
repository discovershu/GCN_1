...
adj, features0, y_train1, _, _, _, _, test_mask0, _, _, _ = read_data.load_data_dc(week=0, test_rat=FLAGS.test_rat, hour=0, k=0)
# Some preprocessing
...
belief_error = []
uncertain_error = []
print("4:", "test_rat:", FLAGS.test_rat)
    ...
    for j in range(16):
        ...
        for k in range(1):
            ...
            for epoch in range(FLAGS.epochs):
                ...
                # load data
                _, features, y_train, _, _, train_mask, _, _, _, _, _ = read_data.load_data_dc(week=p, test_rat=FLAGS.test_rat,
                                                                                        hour=j, k=k)
            ...
            for i in range(weeks):
                i = i + k
                _, features, _, _, y_test, _, _, test_mask, test_index, belief, uncertain = read_data.load_data_dc(week=i, test_rat=FLAGS.test_rat,
                                                                                      hour=j, k=k)
                # features = preprocess_features(features)
                features = sparse_to_tuple(features)
            ...
            b_error, u_error = read_data.get_error_dcpa(opinion_gcn, belief, uncertain, test_index)
            ...
            print("4:", '%02d' % weeks, "test_rat:", FLAGS.test_rat, "belief error=", np.mean(belief_error), "uncertain error=", np.mean(uncertain_error))
