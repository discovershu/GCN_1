...
def get_b_u_6_21(weekday):
    bb = []
    uu = []
    b_all = np.load("/network/rit/lab/ceashpc/xujiang/traffic_data/DC_data/feature/belief_feature_ref0.8.npy")
    u_all = np.load("/network/rit/lab/ceashpc/xujiang/traffic_data/DC_data/feature/uncertainty_feature_ref0.8.npy")
...
def generate_train_test_dc_noise(test_ratio, weekday, noise):
    adj_n = np.load("/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/data/adjacency_matrix_dc_milcom.npy")
    b_all, u_all = get_b_u_6_21(weekday)
    consis_index = get_consistent_index(b_all, adj_n)
    train_mask = []
    test_mask = []
    label = np.zeros(len(adj_n))
    test_num = int(test_ratio * len(label))
    for i in range(len(b_all)):
        train_mask_i, test_mask_i = get_mask(label, test_num)
...
def generate_train_test_pa_noise(test_ratio, weekday, noise):
    adj_n = np.load("/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/data/adjacency_matrix_ph_milcom.npy")
    b_all, u_all = get_b_u_6_21_pa(weekday)
    consis_index = get_consistent_index(b_all, adj_n)
    train_mask = []
    test_mask = []
    label = np.zeros(len(adj_n))
...
def generate_train_test_epinion_noise_sparse(test_ratio, node, noise):
    adj_index = np.load("/network/rit/lab/ceashpc/xujiang/eopinion_data/adj_epinion_47676_index.npy")
    # b_all = np.load("/network/rit/lab/ceashpc/xujiang/eopinion_data/feature/belief_feature_{}.npy".format(node))
...
def generate_train_test_knn(test_ratio, weekday, noise):
    adj_n = np.load("/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/data/adjacency_matrix_ph_milcom.npy")
    b_all, u_all = get_b_u_6_21_pa(weekday)
    consis_index = get_consistent_index(b_all, adj_n)
    train_mask = []
    ...
    adj = adj_n
    return adj, train_feature_b, train_feature_u, train_mask, test_mask, test_ratio


def get_consistant():
    # adj_n = np.load(
        # "/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/traffic_data/epinion_node500_alledges.npy")
    b_all = np.load("/network/rit/lab/ceashpc/xujiang/eopinion_data/feature/belief_feature_47676.npy")
    # u_all = np.load("/network/rit/lab/ceashpc/xujiang/eopinion_data/feature/uncertainty_feature_500.npy")
    neigh = np.load("/network/rit/lab/ceashpc/xujiang/project/GCN_RNN/gae/traffic_data/nodes-47676_neigh.npy")
...
if __name__ == '__main__':
    consis_index = np.load("/network/rit/lab/ceashpc/xujiang/eopinion_data/feature/consis_index_{}.npy".format(47676))
    # generate_train_test_pa_noise(0.2, 1, 0.1)
    print(1)
