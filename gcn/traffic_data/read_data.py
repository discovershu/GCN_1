...
import read_bigdata as bigdata
...
def get_feature_label(feat):
    ...
    for i in range(len(feat)):
        if feat[i] == 0:  # non-conjestion
            ...
            hotvalue[i] = [0, 0]
            unobserve.append(i)
    return hotvalue, feat_n, unobserve
...
def load_data_dc(week, test_rat, hour, k):
    # adj_n = get_adjacency_matrix()
    adj_n = np.load("/network/rit/lab/ceashpc/xujiang/MILCOM/GCN_traffic/gcn/data/adjacency_matrix_dc_milcom.npy")
    features_ = np.load("/network/rit/lab/ceashpc/xujiang/MILCOM/GCN_traffic/gcn/data/feature_dc_0.8_6hours_Fr.npy")
    feature_hour = features_[hour]
    feature_hour_k = feature_hour[:, k:(k + 38)]
    b, u = get_bu(feature_hour_k)
    feature_edge_i = feature_hour[:, week]
    label, feature_n, unob_index = get_feature_label(feature_edge_i)
    random.seed(1)
    test_num = int(test_rat * len(feature_edge_i))
    test_index = random.sample(range(len(feature_edge_i)), test_num)

    index_all = np.concatenate((test_index, unob_index), axis=0)
    ...
    return adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, test_index, b, u


def get_sub_fet(feat):
    feat_n = np.zeros([len(feat), 4])
    for i in range(len(feat)):
        if feat[i][0] == 1:
            feat_n[i] = [1, 0, 0, 0]
        elif feat[i][1] == 1:
...
def load_data_syn_sub(test_rat, index):
    ...
    return adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, test_index

def load_data_epinion(week, T):
    # adj_n = get_adjacency_matrix()
    adj_n = np.load("/network/rit/lab/ceashpc/xujiang/project/GAE_ICDM2018/gae/traffic_data/epinion_node5000_alledges.npy")

    # features_ = np.load("./traffic_data/pa_Fri_0.8.npy")
    # feature_hour = features_[hour]
    # feature_hour_k = feature_hour[:, k:(k+38)]
    # b, u = get_bu(feature_hour_k)
    # feature_edge_i = feature_hour[:, week]

    b, u, test_index, feature_hour = bigdata.get_epinion_data(T)
    feature_hour = np.asarray(feature_hour)
    feature_edge_i = feature_hour[:, week]
    label, feature_n, unob_index = get_feature_label(feature_edge_i)
    # random.seed(132)
    # test_num = int(test_rat * len(feature_edge_i))
...
def load_data_direct(week, test_num, hour):
    ...
    features_ = np.load(
        "/network/rit/lab/ceashpc/xujiang/GAT/GAT1/data/traffic_data/direct_feature_dc_0.8_6hours_Fr.npy")
    feature_hour = features_[hour]
    feature_edge_i = feature_hour[:, week]
    test_index = random.sample(range(len(feature_edge_i)), test_num)
    label, feature_n = get_hotvalue_bigdata(feature_edge_i)
...
def opinion_dcpa(y_list):
    weeks = float(len(y_list))
    y_round = np.around(y_list)
    opinion = np.sum(y_round, axis=0) / (weeks + 2)
    return opinion


def get_error_dcpa(pred, b, u, test_index, T):
    e_b = 0.0
    e_u = 0.0
    for index in test_index:
        e_b += np.abs(pred[index][0] - b[index])
        e_u += np.abs(2.0 / (2.0 + T) - u[index])
    e_b = e_b / len(test_index)
    e_u = e_u / len(test_index)
    return e_b, e_u
