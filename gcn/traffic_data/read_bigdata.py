...
def get_epinion_data(T):
    pkl_file = open(
        "/network/rit/lab/ceashpc/xujiang/eopinion_data/tmp/nodes-5000-T-38-rate-0.1-testratio-0.4-swaprate-0.1-realization-0-data-X.pkl")
    ...
    for i in range(len(E)):
        belief[i] = omega_rs[E[i]][0]
        uncertain[i] = omega_rs[E[i]][2]
        feat_hour.append(t_Obs[E[i]])
        if E[i] in E_X:
            test_index.append(i)

    return belief, uncertain, test_index, feat_hour
...
if __name__ == '__main__':
    ...
    # #     c = np.setdiff1d(nn, [i])
    # b, u = get_dc_data()
    # np.save("dc_belief_0.9.npy", b)
    # np.save("dc_uncertain_0.9.npy", u)
    ff = read_bigdata_re()
    np.save("pa_Wen_0.8.npy", ff)
    for f in ff:
        for item in f:
            if len(item) < 43:
                print("short")
